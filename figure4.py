...
layout = [['a'],
          ...
          ['c']]
plots = [
         dict(label='a', coord='phys', selection=selection, species=1, field='pperp1', vectors=['jx', 'jy'], vector_color=None, cbar=True, legend=True, scalar_cmap=cmap, vector_cmap=None, vector_label=r'$\mathbf{j}_\text{ion}$', scalar_label=r'$\text{p}_{\perp 1, \text{ion}}$'),
         dict(label='b', coord='phys-x', selection=selection, x='x', y='pperp1', y_cut=0, labels=r'$p_{\perp 1, \text{ion}}$', y_label=r'Pressure [nPa]', color='gray', cmap=cmap),
         dict(label='c', coord='phys-x', selection=selection, x='x', y=['jty', 'jtx'], y_cut=0, labels=[r'$\mathbf{j}_{y}$', r'$\mathbf{j}_{x}$'], y_label=r'Current [nA/m$^2$]',
                    color=['orange', 'gray']),
        ]
...
for p in plots:
    ...
    cmap_scalar.set_over(cl.adjust_lightness(cmap_scalar(1.0), 1.4))
    cmap_scalar.set_under(cl.adjust_lightness(cmap_scalar(0.0), 1.4))
    ax.axhline(0, color='white', lw=5, ls='-', alpha=0.6)
...
axes['b'].set_ylabel(r'Pressure [nPa]')
axes['b'].set_ylim([0, 10])
axes['c'].set_ylabel(r'Current [nA/m$^2$]')
axes['c'].set_ylim([-4, 4])
axes['c'].set_yticks(np.arange(-4, 5, 2))
